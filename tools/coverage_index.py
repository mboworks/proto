...
def render_report(summary: dict, target: str) -> str:
    ...
    body = f'    <h1 class="reportTitle">proto coverage: {html.escape(target)}</h1>\n{_full_table(summary)}\n'
    if "patch" in summary:
        patch = summary["patch"]
    ...
    return _page(f"proto coverage: {target}", body)
...
def _short_row(metadata: dict, report_path: str | None = None) -> str:
    ...
    if target == "main":
        label = "main"
        source = '<a href="https://github.com/mboworks/proto/tree/main">main branch</a>'
    elif target.startswith("tag/"):
        release = target.removeprefix("tag/")
        label = f"release {release}"
        source = (
            f'<a href="https://github.com/mboworks/proto/releases/tag/v{html.escape(release)}">'
            f"release v{html.escape(release)}</a>"
        )
    else:
        number = target.removeprefix("pr/")
        label = f"PR {number}"
        source = f'<a href="https://github.com/mboworks/proto/pull/{html.escape(number)}">PR #{html.escape(number)}</a>'
    ...
    if run_id == 0:  # Reports retained before metadata was introduced.
        ...
        commit = (
            f'<a href="https://github.com/mboworks/proto/commit/{html.escape(sha)}">'
            f"<code>{html.escape(sha[:7])}</code></a>"
        )
        attempt = source_metadata["run_attempt"]
        run = f'<a href="https://github.com/mboworks/proto/actions/runs/{run_id}">run {run_id}</a>'
        if attempt > 1:
...
def render_run_history(root: Path) -> str:
    ...
    body = (
        '    <h1>proto pre-merge and post-merge coverage</h1>\n'
    ...
    return _page("proto pre-merge and post-merge coverage", body)
...
"""
    ...
    return _page("proto coverage reports", body)
